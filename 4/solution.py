...
if __name__ == '__main__':
    with open('level4-1.txt', 'r') as in_file:
        ...
        n_hyperloops = int(in_file.readline())
        
        possibleHyperloops = list(itertools.combinations(locations, 2))
        

        for possibleHyperloop in possibleHyperloops:
            key1 = possibleHyperloop[0]
            hyperloop1 = "".join(key1.keys())
            key2 = possibleHyperloop[1]
            hyperloop2 = "".join(key2.keys())
            i = 0
            count = 0
            for _ in range(n_journeys):
                ...
                if(total < currentTimes[i]):
                    count += 1
                i += 1
            if(count == n_hyperloops):
                print(hyperloop2+' '+hyperloop1)
            break
